...
class BCHttpServer(val localHost: LocalHost, bc: BlockChain, peerAccess: PeerAccess, statementsCache: StatementsCache) {
  ...
  def start(nodeName: String): Unit = {
    ...
    server.createContext("/nodeinfo", new NodeInfoHandler(nodeName, this, peerAccess, statementsCache, bc))
    ...
    server.createContext("/approveFact", new ApproveFactHandler(nodeName, this, ProdKeysFileOps, peerAccess, bc, statementsCache))
    server.createContext("/users", new UsersHandler(nodeName, this, bc, ProdKeysFileOps))
    server.start()
